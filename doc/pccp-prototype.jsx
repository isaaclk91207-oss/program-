...
/* =========================================================================
   DESIGN TOKENS (documented, not enforced ΓÇö see inline usage)
   Base:      slate-950 / slate-900 / slate-50 (navy-charcoal professional)
   Accent:    amber-500/600 (certification gold ΓÇö used sparingly)
   Success:   emerald-600   Warning: amber-600   Error: rose-600
   Display:   font-semibold tracking-tight (headers)
   Data/IDs:  font-mono (passport numbers, driver IDs ΓÇö "official document" feel)
   ========================================================================= */

/* -------------------------------------------------------------------------
   MOCK DATA ΓÇö structured as the future API is expected to shape it.
   Scoring model below mirrors CALCULATION_LOGIC_OVERVIEW.docx (CD formula)
   and is explicitly a Phase 1 placeholder ΓÇö weights are configurable.
   ------------------------------------------------------------------------- */
...
const RAW_DRIVERS = [
  ...
  { id: "DRV-006", name: "Zin Min Latt", level: "CD", certStatus: "Pending", status: "Active", validUntil: "ΓÇö", phone: "+95 9 887 4402", email: "zin.min@pccp.demo", vehicle: "Toyota Noah ┬╖ YGN-6631", currentVehicle: "YGN-6631", joined: "03 Jul 2024", accidentFree: "ΓÇö", englishLevel: "A2", credits: 210,
    assessment: { written: 70, practical: 74, operational: 72, feedbackAvg: 4.1 } },
  { id: "DRV-007", name: "Thura Aung", level: "CD", certStatus: "Pending", status: "Active", validUntil: "ΓÇö", phone: "+95 9 320 7754", email: "thura.aung@pccp.demo", vehicle: "Honda Freed ┬╖ YGN-7742", currentVehicle: "YGN-7742", joined: "22 Aug 2024", accidentFree: "ΓÇö", englishLevel: "A2", credits: 150,
    assessment: { written: 65, practical: 68, operational: 70, feedbackAvg: 3.8 } },
  { id: "DRV-008", name: "Nay Lin Oo", level: "CC", certStatus: "Suspended", status: "Inactive", validUntil: "ΓÇö", phone: "+95 9 611 0087", email: "nay.lin@pccp.demo", vehicle: "Toyota Alphard ┬╖ YGN-5590", currentVehicle: "YGN-5590", joined: "11 Nov 2023", accidentFree: "ΓÇö", englishLevel: "B1", credits: 620,
    assessment: { written: 74, practical: 70, operational: 68, feedbackAvg: 3.2 } },
  { id: "DRV-009", name: "Htet Wai Yan", level: "CPC", certStatus: "Revoked", status: "Inactive", validUntil: "ΓÇö", phone: "+95 9 992 3345", email: "htet.wai@pccp.demo", vehicle: "Toyota Vellfire ┬╖ YGN-0043", currentVehicle: "YGN-0043", joined: "05 Dec 2022", accidentFree: "ΓÇö", englishLevel: "B2", credits: 900,
...
/* -------------------------------------------------------------------------
   VEHICLES ΓÇö QR is permanently bound to the car number (plate).
...
/* -------------------------------------------------------------------------
   TRANSPORT REQUESTS
   Status flow: PENDING ΓåÆ ASSIGNED ΓåÆ QR_PENDING ΓåÆ PICK_UP_SCANNED
                ΓåÆ IN_PROGRESS ΓåÆ DROP_OFF_SCANNED ΓåÆ FEEDBACK_SUBMITTED
   ------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
   FEEDBACK RECORDS ΓÇö linked to passenger + driver + vehicle + request
   ------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
   VEHICLE CHECK-INS ΓÇö driver operational records
   ------------------------------------------------------------------------- */
...
function LoadingSpinner({ label = "LoadingΓÇª" }) {
  return (
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-16 bg-slate-950">
      <div className="w-full max-w-4xl">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 mt-3 max-w-lg mx-auto">Choose a demo role to preview its experience. This selector is for prototype demonstration only ΓÇö not the production sign-in.</p>
        </div>
...
function AdminApp({ drivers, setDrivers, vehicles, setVehicles, passengers, transportRequests, setTransportRequests,
  ...
  function addDriver(data) {
    ...
    const d = {
      ...
      vehicle: vehicle ? `${vehicle.make} ${vehicle.model} ┬╖ ${vehicle.plate}` : "ΓÇö",
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-16 bg-slate-950">
      <div className="w-full max-w-sm">
        ...
        <Card className="p-6 space-y-4">
          ...
          <p className="text-[11px] text-slate-400 text-center">Demo credentials pre-filled ΓÇö not a real authentication flow.</p>
        </Card>
              ...
              {transportRequests.slice(0, 5).map((r) => (
                <button key={r.id} onClick={() => onOpenRequest(r.id)} className="w-full flex items-center gap-3 px-5 py-3.5 text-left hover:bg-slate-50">
                  ...
                  <div className="hidden sm:block text-right">
                    <p className="text-xs text-slate-500">{r.pickup} ΓåÆ {r.destination}</p>
            ...
            <tbody className="divide-y divide-slate-100">
              {drivers.slice(0, 5).map((d) => (
                <tr key={d.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => onOpenDriver(d.id)}>
                  ...
                  <td className="px-5 py-3 font-mono text-slate-500">{d.currentVehicle || "ΓÇö"}</td>
                </tr>
              ))}
            </tbody>
          </table>
                ...
                {filtered.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => onOpenPassenger(p.id)}>
                      ...
                      <td className="px-5 py-3 text-slate-700">{p.department || "ΓÇö"}</td>
                      <td className="px-5 py-3 text-slate-700">{p.phone || "ΓÇö"}</td>
                      <td className="px-5 py-3 text-slate-700">{p.email || "ΓÇö"}</td>
                      <td className="px-5 py-3 text-right">
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-5">
      ...
      <Card className="p-6 flex items-center gap-4">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm text-slate-500 mt-1">{passenger.department || "ΓÇö"}</p>
        </div>
      </Card>
      <Card className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <SectionLabel>Phone</SectionLabel>
          <p className="text-sm text-slate-700">{passenger.phone || "ΓÇö"}</p>
        </div>
        <div>
          <SectionLabel>Email</SectionLabel>
          <p className="text-sm text-slate-700">{passenger.email || "ΓÇö"}</p>
        </div>
      </Card>
            ...
            trips.map((t) => (
              <div key={t.id} className="flex items-center justify-between gap-3 p-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500 truncate">{t.pickup} ΓåÆ {t.destination}</p>
                </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select value={form.level} onChange={(e) => set("level", e.target.value)} className="w-full mt-1 rounded-lg border border-slate-200 text-sm py-2.5 px-3 focus:outline-none">
              {CERT_LEVELS.map((l) => <option key={l} value={l}>{l} ΓÇö pass mark {PASS_MARKS[l]}%</option>)}
            </select>
          </div>
          <div>
...
function EditDriverModal({ open, onClose, onSave, driver, vehicles }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Edit Driver ΓÇö ${driver?.name || ""}`}>
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            <label className="text-xs font-medium text-slate-500">Full Name *</label>
...
function AdminDriverDetail({ driver, onBack, onUpdate, onDelete, notify, vehicleCheckins, vehicles, weights, passMarks }) {
  ...
  function handleRevoke() {
    onUpdate({ certStatus: "Revoked", validUntil: "ΓÇö" });
      ...
      <div className="p-6 space-y-5">
        <Card className="p-5 flex flex-col sm:flex-row sm:items-center gap-5">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-400 font-mono mt-1">{driver.id} ┬╖ Level {driver.level} ┬╖ {driver.rating.toFixed(1)} Γÿà</p>
          </div>
...
function AdminAssessmentPanel({ driver, onUpdate, notify, weights = DEFAULT_WEIGHTS, passMarks = PASS_MARKS }) {
  ...
  function validate() {
    ...
    PRACTICAL_CRITERIA.forEach((c) => {
      ...
      else if (Number(val) < 0 || Number(val) > 100) errs[`practical.${c.key}`] = "0ΓÇô100.";
    });
    OPERATIONAL_CRITERIA.forEach((c) => {
      ...
      else if (Number(val) < 0 || Number(val) > 100) errs[`operational.${c.key}`] = "0ΓÇô100.";
    });
    setErrors(errs);
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-5">
      <Card className="p-5 lg:col-span-2">
        <div className="flex items-center justify-between mb-1">
          <SectionLabel>Manual Assessment ΓÇö {driver.name}</SectionLabel>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Derived from monthly average rating ({driver.assessment.feedbackAvg.toFixed(1)} Γÿà ├ù 20).</p>
          </div>
        </div>
      </Card>
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-5">
      ...
      <Card className="p-5 space-y-4">
        ...
        <div className="flex items-center gap-2 pt-1">
          ...
          {!weightsValid && <span className="text-xs text-rose-500">ΓÇö must equal 100%</span>}
        </div>
      </Card>
      <Card className="p-5">
...
/* -------------------------------------------------------------------------
   ADMIN ΓÇö TRANSPORT REQUESTS (list + detail + assignment)
   ------------------------------------------------------------------------- */
      ...
      <div className="space-y-4">
        <div>
          ...
          <select value={driverId} onChange={(e) => onDriverChange(e.target.value)}
            ...
            <option value="">Select a driverΓÇª</option>
            {drivers.map((d) => (
              <option key={d.id} value={d.id}>{d.name} ┬╖ {d.id} ┬╖ Level {d.level} ┬╖ {d.rating.toFixed(1)} Γÿà</option>
            ))}
          </select>
        </div>
        <div>
          ...
          <select value={vehicleId} onChange={(e) => onVehicleChange(e.target.value)}
            className="w-full mt-1 rounded-lg border border-slate-200 text-sm py-2.5 px-3 bg-white focus:outline-none focus:ring-2 focus:ring-slate-300">
            <option value="">Select a vehicleΓÇª</option>
            {vehicles.map((v) => (
              <option key={v.id} value={v.id}>{v.make} {v.model} ┬╖ {v.plate}</option>
            ))}
          </select>
...
/* -------------------------------------------------------------------------
   ADMIN ΓÇö VEHICLES (fleet + permanent vehicle QR)
   ------------------------------------------------------------------------- */
...
function EditVehicleModal({ open, onClose, onSave, vehicle }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Edit Vehicle ΓÇö ${vehicle?.plate || ""}`}>
      <div className="space-y-4">
...
/* -------------------------------------------------------------------------
   ADMIN ΓÇö ASSESSMENTS, OPERATIONAL RECORDS, NOTIFICATIONS
   ------------------------------------------------------------------------- */
...
function AdminAssessments({ drivers, onOpenDriver }) {
  ...
  const buckets = [
    { label: "0 ΓÇô 59", min: 0, max: 59 },
    { label: "60 ΓÇô 69", min: 60, max: 69 },
    { label: "70 ΓÇô 79", min: 70, max: 79 },
    { label: "80 ΓÇô 89", min: 80, max: 89 },
    { label: "90 ΓÇô 100", min: 90, max: 100 },
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3 text-slate-600">
                      {c.checkOutLocation ? <>{c.checkOutLocation}<p className="text-xs text-slate-400">{c.checkOutTime}</p></> : <span className="text-xs text-slate-400">ΓÇö</span>}
                    </td>
...
function DriverApp({ drivers, vehicles, transportRequests, notifications, vehicleCheckins, addVehicleCheckIn, updateVehicleCheckIn, feedbackRecords, markNotificationRead, notify, onExit, userId }) {
  ...
  function handleCheckInDone(data) {
    ...
    addVehicleCheckIn({
      ...
      checkInLocation: data.location, checkInTime: DEMO_TIME, checkInRemark: data.remark || "ΓÇö",
  ...
  function handleCheckOutDone(data) {
    updateVehicleCheckIn(selectedRequest.id, {
      checkOutLocation: data.location, checkOutTime: DEMO_TIME, checkOutRemark: data.remark || "ΓÇö", status: "CHECKED_OUT",
    });
  ...
  if (phase === "scan") {
    return (
      <VehicleQRScan
        ...
        title={isCheckIn ? "Scan Vehicle QR ΓÇö Check In" : "Scan Vehicle QR ΓÇö Check Out"}
  ...
  return (
    <div className="p-5 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 text-center">
        <Card className="p-4">
          <p className="text-2xl font-semibold text-slate-900">{driver.rating.toFixed(1)} Γÿà</p>
...
function DriverProfile({ driver, trips, vehicleCheckins }) {
  ...
  const validUntil = driver.certStatus === "Certified" ? "Jan 2027" : driver.certStatus === "Pending" ? "Pending review" : "ΓÇö";
  return (
    <div className="p-5 space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        <Card className="p-4">
          <p className="text-xl font-semibold text-slate-900">{driver.rating.toFixed(1)} Γÿà</p>
          <p className="text-xs text-slate-400 mt-1">Rating</p>
        </Card>
        <Card className="p-4">
      ...
      <Card className="p-5">
        ...
        <div className="grid grid-cols-2 gap-4 mt-5">
          ...
          <div>
            ...
            <p className="text-xl font-semibold text-slate-900 mt-0.5">{driver.rating.toFixed(1)} Γÿà</p>
          </div>
        </div>
      </Card>
  ...
  return (
    <div className="p-5 flex flex-col items-center">
      <Card className="p-8 flex flex-col items-center w-full">
        ...
        <p className="text-xs text-slate-400 font-mono">{driver.id}</p>
        <p className="text-xs text-slate-500 mt-3">Car plate: <span className="font-mono">{vehiclePlate || "ΓÇö"}</span></p>
    ...
    <div className="p-5 flex flex-col items-center justify-center min-h-[70vh] text-center">
      {state === "scanning" && (
        <>
          ...
          <p className="text-slate-500 text-sm mt-4">Verifying QR codeΓÇª</p>
        </>
      )}
      {state === "success" && (
        <>
          ...
          <Card className="p-5 mt-5 w-full text-left">
            ...
            <InfoRow icon={CreditCard} label="Driver ID" value={driver.id} mono />
            <div className="h-3" />
            <InfoRow icon={Award} label="Certification" value={`${CERT_LEVEL_NAMES[driver.level] || driver.level} ΓÇö Level ${driver.level}`} />
            <div className="h-3" />
    ...
    <div className="p-5 space-y-5">
      ...
      {state === "scanning" && (
        <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
          <Loader2 className="h-10 w-10 text-slate-400 animate-spin" />
          <p className="text-slate-500 text-sm mt-4">Validating QR codeΓÇª</p>
          <p className="text-xs text-slate-400 mt-1">Checking registration, vehicle status and assignmentΓÇª</p>
        </div>
      )}
      {state === "valid" && (
        <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
...
function PassengerApp({ passengers, drivers, vehicles, transportRequests, notifications, addTransportRequest, updateTransportRequest, submitFeedback, markNotificationRead, notify, onExit, userId }) {
  ...
  function renderTripActions(request) {
    ...
    if (request.status === "ASSIGNED" || request.status === "QR_PENDING") {
      return <Button className="w-full" onClick={() => { setScanKind("pickup"); setScreen("scan"); }}><ScanLine className="h-4 w-4" /> Scan Car QR ΓÇö Pick Up</Button>;
    }
    if (request.status === "PICK_UP_SCANNED") {
      return <Button className="w-full" onClick={() => { setScanKind("dropoff"); setScreen("scan"); }}><ScanLine className="h-4 w-4" /> Scan Car QR ΓÇö Drop Off</Button>;
    }
    ...
    if (request.status === "FEEDBACK_SUBMITTED") {
      return (
        <Card className="p-4 bg-emerald-50 border-emerald-200">
          <p className="text-sm text-emerald-800 flex items-center gap-2"><CheckCircle2 className="h-4 w-4" /> Trip completed ΓÇö feedback submitted.</p>
        </Card>
      );
    }
    return null;
    ...
    >
      ...
      {screen === "scan" && selectedRequest && (
        <VehicleQRScan
          ...
          title={scanKind === "pickup" ? "Scan Car QR ΓÇö Pick Up" : "Scan Car QR ΓÇö Drop Off"}
      ...
      <Card className="p-5">
        ...
        <div className="grid grid-cols-3 gap-3 mt-5 text-center">
          <div className="rounded-lg bg-slate-50 py-3">
            <p className="text-lg font-semibold text-slate-900">{driver.rating.toFixed(1)} Γÿà</p>
          ...
          <button key={d.id} onClick={() => onSelect(d)} className="w-full">
            <Card className={cx("p-4 flex items-center gap-4", i === 0 && "ring-1 ring-amber-300 bg-amber-50/40")}>
              ...
              <div className="text-right">
                <p className="text-sm font-semibold text-slate-900">{d.rating.toFixed(1)} Γÿà</p>
      ...
      {rating > 0 && (
        <div>
          ...
          <p className="text-xs text-slate-400 mb-2">
            {rating <= 3 ? "Select the areas that best describe your feedback (1ΓÇô3 stars)." : "Select the areas that best describe your feedback (4ΓÇô5 stars)."}
          </p>
          <div className="flex flex-wrap gap-2">
            {tagBucket.map((tag) => {
